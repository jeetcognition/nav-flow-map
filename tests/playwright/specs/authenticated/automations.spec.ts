...
test.describe("Automations", () => {
  ...
  test("SUB-AU-REG01 — Advanced section renders all controls", async ({ page }) => {
    ...
    // "Agent mode" and "Run as" are labelled controls of the Agent definition
    // section rather than headings of their own.
    ...
    await expect(page.getByText("Run as", { exact: true })).toBeVisible();
    await expect(
