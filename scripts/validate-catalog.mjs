...
function loadScriptConstant(file, name) {
  const context = {};
  vm.createContext(context);
  vm.runInContext(`${fs.readFileSync(file, "utf8")}\nthis.value = ${name};`, context);
  return context.value;
}

JSON.parse(fs.readFileSync(schemaPath, "utf8"));

const bugIds = new Set(loadScriptConstant(path.join(root, "bugs.js"), "BUGS").map((bug) => bug.id));
...
for (const file of files) {
  ...
  for (const testcase of document.testcases) {
    ...
    if (!priorities.has(testcase.priority)) fail(location, `unsupported priority ${testcase.priority}`);
