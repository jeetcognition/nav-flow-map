...
export async function fetchLatestOtp(options: FetchOtpOptions = {}): Promise<string> {
  ...
  if (!user || !password) {
    throw new Error('Gmail OTP: missing inbox credentials. Set GMAIL_IMAP_USER (or DEVIN_ADMIN_EMAIL) and GMAIL_APP_PASSWORD in .env.');
  }
  ...
  try {
    ...
    while (Date.now() < deadline) {
      const code = await findNewestCode(client, { fromIncludes, subjectIncludes, codeRegex });
      if (code) return code;
...
async function findNewestCode(
  ...
  try {
    ...
    const msg = await client.fetchOne(String(maxUid), { envelope: true, source: true }, { uid: true });
 ...
 */
export async function deleteOtpEmails(options: { user?: string; password?: string; subjectPattern?: RegExp } = {}): Promise<number> {
  const user = options.user ?? envDefault('GMAIL_IMAP_USER') ?? envDefault('DEVIN_ADMIN_EMAIL');
  const password = options.password ?? envDefault('GMAIL_APP_PASSWORD');
