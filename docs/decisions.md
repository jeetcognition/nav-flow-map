...
## QA-DEC-027 — Pattern engine + pattern-first Incidents UI (best-of-both intake flow)

- **Date:** 2026-08-07
- **Status:** Accepted
- **Question:** The intake pipeline judges tickets one at a time (QA-DEC-025), so the Incidents feed cannot answer "which breaking workflow has no test coverage?" — the core question the standalone `ent-qa/pylon-report-parser` answers with graph clustering. How do we combine both without violating this repo's invariants (PII never leaves the runner, no LLM at inference, all writes via PR, eval-gated rules)?
- **Answer:** (1) A deterministic pattern engine (`pipelines/pylon/pattern_engine.py`, adapted from the report parser's DSU clustering) groups classifier-approved tickets (`definite-bug`/`possible-bug` only) into patterns per surface+flow, using normalized titles, language-agnostic error signatures, and token-Jaccard. (2) Pattern _memory_ is DERIVED from the 60-day window each run (first_seen = oldest member ticket, growth = last-24h vs previous-24h), so no mutable state must survive CI runs — unlike the parser's force-pushed `data` branch, which would violate the "DB never leaves the runner" invariant. (3) The only true state is human feedback, stored in the committed, sanitized `pipelines/pylon/coverage.json` ledger ({pattern_id → uncovered/weak/covered/dismissed + coveredBy}), same append-through-PR philosophy as `labels/eval_set.json`. (4) `export_incidents.py` additionally emits `app/src/data/fixtures/patterns.json` (sanitized labels, evidence as ticket numbers + https links, member incident ids); `scripts/validate-data.js` extends referential checks and the PII leak scan to it. (5) The Incidents page is pattern-first: view rail (Coverage gaps default / Verify queue / All patterns / Covered wins / Dismissed noise), digest strip, product chips top-right, expandable pattern cards with what-breaks/what-to-test/evidence and the individual tickets folded inside; the old flat feed, four filter dropdowns, escaped-defect strip, and `IncidentBreakdown` charts are removed (user decision, 2026-08-07 Lavish review). (6) One language-agnostic classifier rule (`ref-id-evidence`: reference ids like `ORFVDYCI-0011` + invoice-number words in CJK) was added under the QA-DEC-025 refiner protocol — eval before: P 98% / R 98% / F1 0.98; after: P 98% / R 100% / F1 0.99, definite band 14/14 both, no previously-passing ticket flipped, gate green.
- **Rationale:** The parser's pattern brain answers the money question but has no accuracy exam and a PII-hostile persistence model; this repo has the proven judge and the discipline but no pattern view. Deriving temporal memory from the retention window removes the need for cross-run state entirely, and a committed coverage ledger keeps human feedback auditable and PR-reviewed like every other write.
- **Implementation:** `pipelines/pylon/pattern_engine.py`, `pipelines/pylon/coverage.json`, `pipelines/pylon/export_incidents.py` (`export_patterns`), `pipelines/pylon/fetcher.py` (day-chunked + retention-clamped incremental fetch, retention cleanup each run), `pipelines/pylon/ticket_classifier.py` (`ref-id-evidence`), `pipelines/pylon/test_pipeline.py` (pattern-engine suite), `scripts/validate-data.js`, `.github/workflows/pylon-intake.yml`, `app/src/data/fixtures/patterns.json`, `app/src/types.ts` (`Pattern`), `app/src/data/dataService.ts` (`getPatterns`/`patternIncidents`/`setPatternCoverage`), `app/src/pages/Incidents.tsx`, `app/src/components/incidents/{PatternCard,VerifyQueue,SurfaceChips}.tsx`, `app/src/styles/incidents.css`.

## QA-DEC-028 — Verdict persistence: UI → worker → committed ledgers

- **Date:** 2026-08-07
- **Status:** Accepted
- **Question:** Coverage verdicts (Mark covered / Dismiss) and verify-queue verdicts (Confirm bug / Not a bug) were local-first store mutations that vanished on reload; QA-DEC-027 left promotion into `coverage.json` and the refiner's gold labels manual. What is the persistence path, and does a human verdict commit straight to `main` or open a PR?
- **Answer:** (1) A new worker route `POST /verdicts` (same origin-allowlisted Cloudflare worker that saves navmap edits) accepts `{coverage: {pattern_id → {status, coveredBy, by}}, tickets: {ticket_number → {category, by}}}`, validates shapes against allowlists, scrubs email-like strings, stamps timestamps server-side, and read-merge-writes two committed ledgers via the GitHub contents API (one retry on sha races): `pipelines/pylon/coverage.json` and `pipelines/pylon/labels/pending_verdicts.json`. (2) Human verdicts commit **directly to main**, following the `navmap-edits.json` precedent: they are human actions relayed by the worker, not automated writes — the PR-only invariant applies to pipeline/AI writers. Neither ledger can redden main: the eval gate never reads `pending_verdicts.json` (the refiner folds it into the append-only `eval_set.json` under its own gated PR), and both files are shape-validated by the worker on write and by `scripts/validate-data.js` in CI. (3) Ticket verdicts store the full `IncidentCategory` (not a collapsed bug/not label) so the fixture round-trip is lossless; the refiner derives eval labels (app-bug → bug; customer-doubt/config-issue/feature-request → not; unknown → unsure). (4) In the app, `data/verdictsService.ts` is the wire layer behind the existing `dataService` mutations: verdicts apply to the store instantly, queue in localStorage (surviving reloads), flush debounced as one batch, and surface sync state through `VerdictSyncBanner` (saving / N-not-saved + retry — no silent drops). On load, both ledgers are overlaid onto the fixtures (`loadVerdictBaselines`), so verdicts survive reloads even before the next pipeline export re-bakes them; `export_incidents.py` applies pending verdicts as `humanCategory`/`overriddenBy` so exports converge with what humans decided. (5) The worker→pipeline contract is tested hermetically: `worker/test_verdicts.mjs` (GitHub API stubbed; scrub, merge, validation, origin checks) and `test_pipeline.py` (ledger loaders fail open on malformed data) — both run in the Validate workflow.
- **Rationale:** Verdicts are the scarcest input in the whole system — a human judgment call — so the path must not lose them (queue + retry + reload survival) and must not launder them through mutable app state (committed ledgers are the source of truth, auditable in git history like every other decision). Direct-to-main is acceptable exactly because the ledgers are display/queue data that cannot break a gate; everything that CAN move the classifier still goes through the refiner's eval-gated PR.
- **Implementation:** `worker/worker.js` (`/verdicts`, `mergeLedger`), `worker/test_verdicts.mjs`, `app/src/data/verdictsService.ts`, `app/src/data/dataService.ts` (`setPatternCoverage`/`overrideIncidentCategory` enqueue, `loadVerdictBaselines`), `app/src/components/incidents/VerdictSyncBanner.tsx`, `app/src/pages/{Incidents,IncidentDetail}.tsx`, `app/src/lib/config.ts` (`VERDICTS_ENDPOINT`, `COVERAGE_JSON_URL`, `PENDING_VERDICTS_URL`), `pipelines/pylon/labels/pending_verdicts.json`, `pipelines/pylon/export_incidents.py` (`load_pending_verdicts`), `pipelines/pylon/test_pipeline.py`, `scripts/validate-data.js`, `.github/workflows/validate.yml`. Requires `wrangler deploy` to take effect.
