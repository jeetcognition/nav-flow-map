...
devinBrowser/            Devin session runner controlling Chrome (planned migration)
...
- `devinBrowser` runs control Chrome in a Devin session, consume catalog cases, and write immutable results; they do not edit definitions while executing.
...
Additional application and CLI testcase surfaces are future extensions, not part of the current execution scope.
